...
import ru.lightside.relaxpaint.event.UiEvent
import ru.lightside.relaxpaint.viewmodels.AddWordViewModel
...
fun AddWordScreen(
    ...
    // Состояние для управления показом диалога
    ...
    if (showDialog) {
        AlertDialog(
            onDismissRequest = { },
            title = { Text("Внимание") },
            text = { Text("Изменения не будут сохранены. Вернуться?") },
            confirmButton = {
                TextButton(onClick = {
                    onBack()
                }) {
    ...
    Scaffold(
        ...
        Column(modifier = Modifier
            ...
            OutlinedTextField(
                ...
                onValueChange = { viewModel.onOriginalChange(it)  },
                label = { Text("Слово") },
                modifier = Modifier.fillMaxWidth()
            )
            Spacer(modifier = Modifier.height(8.dp))
            OutlinedTextField(
                value = viewModel.translation,
